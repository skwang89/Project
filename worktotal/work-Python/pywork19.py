...
# 예를 들어, 검색엔진을 구현할 때 사용하는 크롤러는 웹 사이트의 링크를 타고
...
# 웹에 공개된 정보는 대부분 HTML 형식. 이를 가져와서 데이터베이스에
# 저장 하려면 데이터 가공이 필요하다.
...
# 제공합니다. 또한 인증, 리다이렉트, 쿠키(Cookie) 처럼 인터넷을 이용한 다양한
# 요청과 처리를 지원합니다.
# 4. BeautifulSoup 라이브러리를 이용하면 간단하게 HTML과 XML에서 정보를
# 추출할 수 있습니다.
...
# 자바스크립트를 이 사용하는 웹 사이트의 경우에는 앞에서 소개한 requests
